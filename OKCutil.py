...
def ParseBasics(text):
    ...
    for category in listvals:
        ...
        for kw in basicskws[category]:
            match = regex.findall(r'\W('+kw+')\W', text)
            if any(match):
...
def ParseBackground(text):
    ...
    for category in list(filter(lambda key: key not in listvals, 
        ...
        if not any(value):
            ...
            df[category] = value
            text = regex.sub(value[0], '', text)

    secondary = regex.findall('(?<=some\s)(\w+)', text)
    if not any(secondary):
        df['lang_secondary'] = None
    else:
        df['lang_secondary'] = [[lang for lang in secondary]]

    text = regex.sub('some\s\w+|speaks|and|but|it\u2019s', '', text)

    primary = regex.findall('\w+', text)
    if not any(primary):
        df['lang_parimary'] = None
    else:
...
def ParseMisc(text):
    ...
    matches = [regex.findall(r'\w+(?= '+terms[0]+')|(?<='+terms[0]+' )\w+', text),
               regex.findall(r'\w+(?= '+terms[1]+')|(?<='+terms[1]+' )\w+', text),
               regex.findall(r'[\w(?:\u2019)]+(?= '+terms[2]+')|(?<='+terms[2]+' )[\w(?:\u2019)]+', text),
              ]
...
def ParseLookingfor(text):
    ...
    #df['lf_dist'] = [int(dist[0]) if any(dist) else -1]

    df['lf_age_lower'] = [int(regex.findall(r'\d+(?=\u2011)', text)[0])]
    ...
    df['lf_monogamous'] = [kw if kw in text else None for kw in lookingkws['lf_monogamous']]
